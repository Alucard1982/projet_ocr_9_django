...
def flux(request):
    ...
    posts = sorted(
        chain(all_ticket, all_review), key=lambda post: post.time_created, reverse=True
    )
...
def post(request):
    ...
    posts = sorted(
        chain(own_ticket, own_review), key=lambda post: post.time_created, reverse=True
    )
    context = {"posts": posts}
...
def post_user_following(request):
    ...
    if request.method == "POST":
        ...
        try:
            users_following_ticket = Ticket.objects.filter(
        ...
        try:
            ...
            raise Http404("Review does not exist")
        followed_ticket = users_following_ticket.annotate(
            content_type=Value("TICKET", CharField())
        )
        followed_review = followed_review.annotate(
            content_type=Value("REVIEW", CharField())
        )
        posts = sorted(
            chain(followed_ticket, followed_review),
...
def ticket(request, id_ticket=None):
    instance_ticket = (
        get_object_or_404(Ticket, pk=id_ticket) if id_ticket is not None else None
    )
    if request.method == "POST":
...
def review(request, id_ticket=None, id_review=None):
    ...
    instance_review = (
        get_object_or_404(Review, pk=id_review) if id_review is not None else None
    )
    if request.method == "POST":
    ...
    if request.method == "POST":
        ...
        if form_review.is_valid():
            try:
                ...
                raise Http404("Ticket does not exist")
            sorted_instance_ticket = sorted(
                instance_ticket, key=lambda ticket: ticket.pk, reverse=True
            )
